...
const event = {
  ...
  TS: Math.floor(Date.now() / 1000), // Store as Unix timestamp
...
function sendDataToMQTTEvery5Seconds() {
  setInterval(() => {
    const event = generateRandomData();
    const topic = "apm/device/events";

    console.log("Publishing event data:", JSON.stringify(event)); // Debugging log

    device.publish(topic, JSON.stringify(event), (err) => {
      if (err) {
        console.error("Error publishing to MQTT:", err);
      } else {
        console.log("Data published to MQTT topic:", topic);
      }
    });
